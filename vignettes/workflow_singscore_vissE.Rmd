...
# subset dataset to extract only control samples of PMC42-ET and PMC42-LA for DE analysis
emt_dge_sub = emt_dge[,emt_dge$samples$Subline %in% c("PMC42-ET","PMC42-LA") &
                        emt_dge$samples$Treatment == "control"]
head(emt_dge_sub$samples)
```

### create model design matrix
```{r}
# create model design matrix
design = model.matrix(~ Subline, data = emt_dge_sub$samples)
design
```

```{r}
# preprocess, filter by expression. 
keep = filterByExpr(emt_dge_sub, design = design)
table(keep)

# filter by expression.
emt_dge_sub = emt_dge_sub[keep, ]

# calculate normalisation factor using on TMM (default).
emt_dge_sub = calcNormFactors(emt_dge_sub,method = "TMM")
head(as.data.frame(emt_dge_sub$samples))

# Estimate dispersions
emt_dge_sub = estimateDisp(emt_dge_sub, design = design, robust = TRUE)

```

### Differential expression Analysis using edgeR-GLM
```{r, message=FALSE, warning=FALSE}

# fit usimg edgeR's Generalized Linear Models with Quasi-likelihood Tests (GLM) function.
fit = glmQLFit(emt_dge_sub, design, robust = TRUE)
par(mfrow = c(1, 2))
plotMD(fit)
plotQLDisp(fit)
...
### Geneset Enrichment Analysis (limma fry)
```{r, message=FALSE, warning=FALSE}

#Use geneset collections (genesigs) generated earlier in the Singscore:multiscore section
#to generate indices
fry_indices = ids2indices(geneIds(genesigs), rownames(emt_dge_sub))
length(fry_indices)

#remove small and very large genesets
fry_indices = fry_indices[sapply(fry_indices, length) > 10 
                          & sapply(fry_indices, length) < 500]
length(fry_indices)

# remove any empty indices.
empty = sapply(fry_indices, length) == 0
fry_indices = fry_indices[!empty]
length(fry_indices)

#Perform a geneset enrichment analysis using limma::fry
fry_res = fry(emt_dge_sub, index = fry_indices, design = design,contrast = 'SublinePMC42-LA' )

#filter out non-significant results
fry_res = fry_res[fry_res$FDR <0.05,]
head(as.data.frame(fry_res))
```

### Summarising results of gene-set enrichment analysis 
```{r, message=FALSE, warning=FALSE}
...
gs_ovnet = computeMsigNetwork(gs_ovlap, msigdb.hs)
...
The overlap network plot above is annotated based on the MSigDB categories utilised. As we have previously generated gene-set statistics using singscore::multiscore, these can be projected onto the network.
...
#plot the top 6 clusters
set.seed(21) #set seed for reproducible layout
p1 = plotMsigNetwork(gs_ovnet, markGroups = grps[1:6], genesetStat = gsStats)
p1
```
...
p2 = plotMsigWordcloud(msigdb.hs, grps[1:12], type = 'Name')
p2

#using gene-set Short descriptions
plotMsigWordcloud(msigdb.hs, grps[1:12], type = 'Short')
...
p3 = plotGeneStats(gStats, msigdb.hs, grps[1:12]) +
...
library(patchwork)
p2+labs(subtitle = 'Wordcloud') + 
  p1 +labs(subtitle = 'Network') + 
  p3 +labs(subtitle = 'gene-level statistics')
```
# Summary
